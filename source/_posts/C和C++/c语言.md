...
以上就是 `stdio.h` 库的函数详细总结。希望能够帮助到你！
...
在 C 语言中，动态库是一种特殊类型的二进制文件，它包含可以在运行时被多个程序共享的函数和数据。动态库的主要优点是它可以被多个正在运行的程序共享，从而减少了内存使用和磁盘空间，并且它允许你在不重新编译和链接主程序的情况下更新库中的代码。在不同的操作系统中，动态库可能有不同的文件扩展名，例如在 Windows 中是 .dll（动态链接库），在 Unix 或 Linux 中是 .so（共享对象）。

1. **创建动态库**

在 GCC 中，你可以使用 `-shared` 选项来创建一个动态库。例如，假设你有一个名为 `lib.c` 的源文件，你可以使用以下命令来创建一个动态库：

```shell
gcc -shared -o lib.so lib.c  #linux
gcc -shared -o lib.dll lib.c # windows
```

2. **使用动态库**

动态库可以在编译时（编译时链接）或运行时（运行时链接）被加载和链接。编译时链接是指在编译和链接主程序时，编译器和链接器将动态库链接到主程序中。运行时链接是指在主程序运行时，主程序动态地加载和链接动态库。
...
在运行时链接，你需要在你的代码中使用特定的 API 来打开动态库，查找符号（例如函数或变量的地址），然后调用这些函数或访问这些变量。在 Unix 或 Linux 中，这个 API 通常是 dlopen, dlsym, dlclose 等函数。在 Windows 中，这个 API 是 LoadLibrary, GetProcAddress, FreeLibrary 等函数。
