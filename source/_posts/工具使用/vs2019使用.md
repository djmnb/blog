...
3. `.lib` 文件：这是导入库（import library）。**当你的程序在编译时链接到这个DLL时**，链接器会使用这个.lib文件来找到DLL中的函数和类。.lib文件包含了DLL中导出的函数和类的名称以及它们在DLL中的位置。你可以把.lib文件看作是一个索引，链接器可以用它来在DLL中找到函数和类。**链接成功后, 其实这个文件就可有可无了**
